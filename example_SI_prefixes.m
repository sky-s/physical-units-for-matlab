...
% Use u.get('prefixunit') due to MATLAB limitations with constant properties

%% Basic Examples
fprintf('=== SI Prefix Examples ===\n\n');

% These all work with u.get() or static methods:
fprintf('Length units:\n');
fprintf('  1 kilometer = %g meters\n', double(1 * u.get('kilometer') / u.meter));
fprintf('  1 millimeter = %g meters\n', double(1 * u.get('millimeter') / u.meter));
fprintf('  1 nanometer = %g meters\n', double(1 * u.get('nanometer') / u.meter));

fprintf('\nElectrical units:\n');
fprintf('  1 kilovolt = %g volts\n', double(1 * u.get('kilovolt') / u.volt));
fprintf('  1 milliampere = %g amperes\n', double(1 * u.get('milliampere') / u.ampere));
fprintf('  1 microfarad = %g farads\n', double(1 * u.get('microfarad') / u.farad));

fprintf('\nPower and Energy:\n');
fprintf('  1 megawatt = %g watts\n', double(1 * u.get('megawatt') / u.watt));
fprintf('  1 kilojoule = %g joules\n', double(1 * u.get('kilojoule') / u.joule));
fprintf('  1 gigawatt = %g watts\n', double(1 * u.get('gigawatt') / u.watt));

fprintf('\nFrequency:\n');
fprintf('  1 kilohertz = %g hertz\n', double(1 * u.get('kilohertz') / u.hertz));
fprintf('  1 megahertz = %g hertz\n', double(1 * u.get('megahertz') / u.hertz));
fprintf('  1 gigahertz = %g hertz\n', double(1 * u.get('gigahertz') / u.hertz));
...
current = 5 * u.get('milliampere');    % 5 mA
...
freq = 2.4 * u.get('gigahertz');       % 2.4 GHz (WiFi frequency)
...
for i = 1:length(prefixes)
    ...
    try
        unit_val = u.get(unit_name);  % Use u.get() instead of direct access
...
try
    ...
    fprintf('  u.T = %s (tesla)\n', char(u.T));
    fprintf('  Use u.get(''megawatt''), u.get(''terawatt'') instead of MW, TW\n');
catch ME
...
fprintf('with any base unit using u.get(''prefixunit'') or static methods.\n');
fprintf('This greatly expands the available units without cluttering\n');
fprintf('the u class with explicit definitions.\n');